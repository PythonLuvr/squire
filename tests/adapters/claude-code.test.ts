// Claude Code adapter coverage. Feeds a captured stream-json fixture and
// asserts the adapter produces the expected discriminated-union events.
//
// The fixture at tests/fixtures/claude-code/list-files.jsonl was derived
// from a real `claude --output-format stream-json --verbose --print` run
// against a temp dir containing alpha.txt, beta.txt, gamma.txt. It has been
...
const fixturePath = join(here, "..", "fixtures", "claude-code", "list-files.jsonl");

function runAgainstFixture(): SquireEvent[] {
  const text = readFileSync(fixturePath, "utf8");
  const instance = claudeCodeAdapter.create({ binary: "claude", args: [] });
  // Push in two arbitrary chunks to exercise the line-buffer split logic.
...
test("claude-code adapter: at least one non-empty text_delta", () => {
...
test("claude-code adapter: at least one tool_call with non-empty name", () => {
  const events = runAgainstFixture();
...
test("claude-code adapter: tool_result id matches its tool_call id", () => {
  const events = runAgainstFixture();
  const calls = events.filter((e): e is Extract<SquireEvent, { type: "tool_call" }> => e.type === "tool_call");
...
test("claude-code adapter: surfaces thinking_delta when present", () => {
  const events = runAgainstFixture();
  const thinkings = events.filter((e) => e.type === "thinking_delta");
  assert.ok(thinkings.length > 0, "fixture contains a thinking block; adapter must emit thinking_delta");
});

test("claude-code adapter: emits usage event with at least one counter", () => {
  const events = runAgainstFixture();
...
test("claude-code adapter: unparseable lines fall back to stdout (no crash)", () => {
  const instance = claudeCodeAdapter.create({ binary: "claude", args: [] });
  const events = instance.onStdout("this-is-not-json\n");
  // First event is the raw `stdout` chunk; second is the parse fallback.
